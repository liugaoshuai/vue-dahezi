    ...
    <div class="">
        <div class="pd10 bgbai">
            <div class="">
                ...
                <ul class="clearfix">
                    <li class="carpay-item-box"
                        v-for="item in cardClassList"
                        @click="getPayClass(item.id,item.salePrice)"
                        :key="item.id">
                        <div class="common-item carpay-item"
                             v-bind:class="{active: cardClassActvie == item.id}">
                            <p class="fs16"
                               v-text="item.name">{{item.name}}</p>
                            <p class="fs12"
                               v-text="item.description?item.description:'洗车项目说明'">{{item.description?item.description:'洗车项目说明'}}</p>
                            <p class="fs24"
                               v-text="'¥' + item.salePrice"></p>
                        </div>
                    </li>
                </ul>
            </div>
            ...
            <div class="box-content mgb10 bgbai">
                ...
                <ul class="box-content-class clearfix">
                    <li class="carpay-pay common-item"
                        v-if="ciCard.name"
                        @click="payClassClick(1,ciCard.id)"
                        v-bind:class="{active: payClassActvie == 1}">
                    ...
                    <li class="carpay-pay common-item"
                        v-if="yueCard.name"
                        @click="payClassClick(2,yueCard.id)"
                        v-bind:class="{active: payClassActvie == 2}">
                    ...
                    <li class="carpay-pay common-item"
                        v-if="juanCard.name"
                        @click="payClassClick(3,juanCard.id)"
                        v-bind:class="{active: payClassActvie == 3}">
                    ...
                    <li class="carpay-pay common-item"
                        @click="payClassClick(4,0)"
                        v-bind:class="{active: payClassActvie == 4}">
                        <p class="fs14">支付</p>
            ...
            <div class="carpay-footer">
                ...
                <div class="fs12">点击确认下单，即已同意
                    <span class="color-yellow"
                          @click="goXieyi">《大盒子服务协议》</span>
                </div>
                <div class="common-btn mgt10"
                     @click="getPay">确认下单</div>
                <div class="fs12 mgt10">
                    <span class="color-yellow"
                          @click="goPayMonthCard">购买月卡</span>洗车无限次</div>
            </div>
        </div>
    </div>
</template>
...
export default {
    ...
    methods: {
        ...
        getCarClass: function () {
            var self = this,
                url = 'http://test.yixiutong.cn/wxcw/scan/deviceservice.json',
                params = {
                    deviceId: 1002,
                },
        ...
        getPayClass: function (id, p) {
            console.log(id, p)
            var self = this,
                url = 'http://test.yixiutong.cn/wxcw/couponcard/querymatches.json',
                params = {
                    serviceId: id,
                    price: p
                },
        ...
        getPay: function () {
            ...
            var self = this,
                url = 'http://test.yixiutong.cn/wxcw/scan/cwpay.json',
                params = {
        ...
        getWeiXinPay: function (data) {
            var self = this,
                url = 'http://test.yixiutong.cn/wxcw/pay/prepayinfo.json',
                params = {
                ...
                WeixinJSBridge.invoke(
                    ...
                    function (res) {
                        if (res.err_msg == "get_brand_wcpay_request:ok") { 
                            self.$router.push({ path: '/AttentionMatters', query: { orderId: data.orderId } })
